...
Public Class SeedData
    ...
    Public Sub InitializeDbWithValues(context As DataContext)
        ...
        'Else
        SeedFromFile(context)   ' uses JSON given or default
        'End If
    ...
    Private Sub SeedFromFile(context As DataContext)
        ...
        Dim jsonPersons As List(Of VBLib.Person)

        jsonPersons = VBLib.PersonHelpers.DeserializeJSON(jsonFile_)
        ...
        Dim mvcPersons = New List(Of Adapters.Person)

        Dim pId As Integer = 0

        For Each person In jsonPersons
            If (person.IsValid) Then
                mvcPersons.Add(New Adapters.Person(person, pId))
                pId = pId + 1
            End If
        Next
        ...
        mvcPersons.ForEach(Function(g) context.Persons.Add(g))
    ...
    '    Dim psnList = New List(Of Adapters.Message) From {
    ...
    Public Sub New()
        jsonFile_ = HttpContext.Current.Server.MapPath("~/App_Data/indented_dpsvba-w14.json")
    End Sub
